const url=new URL('cr.wasm',import.meta.url);
...
const src=()=>`(async()=>{
  ...
  const fn=({data,width,height,w,h})=>{
    try{
      ...
      wasm.resize(r,p1,n1,width,height,w,h);
 ...
 */
const resize=(data,sourceWidth,sourceHeight,targetWidth,targetHeight)=>new Promise(r=>{
  worker.onmessage=msg=>{
    worker.onmessage=null;
    r(msg.data);
  }
  worker.postMessage({data,width:sourceWidth,height:sourceHeight,w:targetWidth,h:targetHeight});
});
export {
  resize
};
